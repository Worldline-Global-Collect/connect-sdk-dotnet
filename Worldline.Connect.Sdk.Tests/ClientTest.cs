...
namespace Worldline.Connect.Sdk
{
    ...
    public class ClientTest
    {
        ...
        {
            var client1 = Factory.CreateClient(FactoryTest.Dict, FactoryTest.ApiKeyId, FactoryTest.SecretApiKey);
        ...
        private static void AssertClientHeaders(Client client, string clientMetaInfo)
        {
        ...
        private static void AssertNoClientHeaders(Client client)
        {
            var headers = GetHeaders(client);
            Assert.IsEmpty(headers);
        }

        private static IEnumerable<RequestHeader> GetHeaders(Client client)
        {
