...
namespace Worldline.Connect.Sdk.Webhooks
{
    ...
    {
        private static bool SignaturePredicate(IRequestHeader h) => h.Name.Equals("X-GCS-Signature", StringComparison.InvariantCultureIgnoreCase);
        private static bool KeyIdPredicate(IRequestHeader h) => h.Name.Equals("X-GCS-KeyId", StringComparison.InvariantCultureIgnoreCase);

        private readonly ISecretKeyStore _secretKeyStore;

        public SignatureValidator(ISecretKeyStore secretKeyStore)
        {
            _secretKeyStore = secretKeyStore ?? throw new ArgumentException("secretKeyStore is required");
        }
        ...
        private void ValidateBody(byte[] body, IEnumerable<IRequestHeader> requestHeaders)
        {
            ...
            {
                mac.Initialize();
                var unencodedResult = mac.ComputeHash(body);
                var expectedSignature = Convert.ToBase64String(unencodedResult);
                var isValid = signature.CompareWithoutTimingLeak(expectedSignature);
                if (!isValid)
                {
